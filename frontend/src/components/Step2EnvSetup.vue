...
interface ProfilesRealtimeData {
  profiles?: AgentProfile[]
  total_expected?: number
}

interface ConfigRealtimeData {
...
const emit = defineEmits<{
  (e: 'go-back'): void
  (e: 'next-step', params?: { maxRounds?: number }): void
...
const simulationConfig = ref<SimulationConfigData | null>(null)
...
const autoGeneratedRounds = computed((): number | null => {
...
// 计算所有人设的关联话题总数
const totalTopicsCount = computed((): number => {
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      const data = res.data as PrepareStatusData & { already_prepared?: boolean; task_id?: string; expected_entities_count?: number; entity_types?: string[] }
      ...
      if (data.task_id) {
        addLog(t('log.prepareTaskId', { taskId: data.task_id }))
      }
      
      // 立即设置预期Agent总数（从prepare接口返回值获取）
      if (data.expected_entities_count) {
        expectedTotal.value = data.expected_entities_count
        addLog(t('log.zepEntitiesFound', { count: data.expected_entities_count }))
        if (data.entity_types && data.entity_types.length > 0) {
          addLog(t('log.entityTypes', { types: data.entity_types.join(', ') }))
        }
      }
    ...
    addLog(t('log.prepareException', { error: errorMessage }))
    emit('update-status', 'error')
  }
}
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      const data = res.data as PrepareStatusData
      ...
      if (data.progress_detail) {
        ...
        if (logKey !== lastLoggedMessage && detail.item_description) {
          ...
          if (detail.total_items && detail.total_items > 0) {
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      const data = res.data as ProfilesRealtimeData
      const prevCount = profiles.value.length
      profiles.value = data.profiles || []
      // 只有当 API 返回有效值时才更新，避免覆盖已有的有效值
      if (data.total_expected) {
        expectedTotal.value = data.total_expected
      }
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      const data = res.data as ConfigRealtimeData
      ...
      if (data.config_generated && data.config) {
        ...
        // 显示详细配置摘要
        if (data.summary) {
          addLog(t('log.configSummaryAgents', { count: data.summary.total_agents || 0 }))
          addLog(t('log.configSummaryHours', { hours: data.summary.simulation_hours || 0 }))
          addLog(t('log.configSummaryPosts', { count: data.summary.initial_posts_count || 0 }))
          addLog(t('log.configSummaryTopics', { count: data.summary.hot_topics_count || 0 }))
        ...
        if (data.config.time_config) {
          ...
          addLog(t('log.timeConfigDetail', { minutes: tc.minutes_per_round || 0, rounds: Math.floor(((tc.total_simulation_hours || 0) * 60) / (tc.minutes_per_round || 1)) }))
        }
...
const loadPreparedData = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          addLog(t('log.configSummaryAgents', { count: data.summary.total_agents || 0 }))
          addLog(t('log.configSummaryHours', { hours: data.summary.simulation_hours || 0 }))
          addLog(t('log.configSummaryPostsAlt', { count: data.summary.initial_posts_count || 0 }))
        }
    ...
    const errorMessage = err instanceof Error ? err.message : String(err)
    addLog(t('log.loadConfigFailed', { error: errorMessage }))
    emit('update-status', 'error')
  }
}
